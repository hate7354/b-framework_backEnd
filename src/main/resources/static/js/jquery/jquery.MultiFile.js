		...
		this
			...
			.each(function () {
				...
				MultiFile.addSlave = function (slave, slave_count) {
					...
					/*********************************************************************************
					 * 파일 첨부 필드 클론 생성에 따라 label 변경.
					 *********************************************************************************/
					if($(".MultiFile-wrap#"+MultiFile.wrapID).children("label").length > 0){
						$(".MultiFile-wrap#"+MultiFile.wrapID+" label").attr("for", slave.id);
					}else{
						$(".MultiFile-wrap#"+MultiFile.wrapID).append("<label for=\""+slave.id+"\"><i class=\"fas fa-folder-open\"></i><span>파일선택</span></label>");
					}
...
//										if ( v == x || v == x.substr(x.length - v.length)) {
						...
						if (( MultiFile.max != -1 && $(this).parent().next().children("ul:eq(0)").children("li:not(.hide)").length >= MultiFile.max) || MultiFile.max>0 && prevs.length + files.length > MultiFile.max) {
